...
void consultarExtratoADM(struct Investidores *investidores, int qtd_investidores)
{
    if (qtd_investidores == 0)
    {
        printf("Nenhum investidor cadastrado.\n");
        return;
    }

    char nome_investidor[50];
    printf("Digite o nome do investidor: ");
    if (fgets(nome_investidor, sizeof(nome_investidor), stdin) == NULL)
    {
        printf("Erro ao ler o nome, tente novamente.\n");
        return;
    }
    nome_investidor[strcspn(nome_investidor, "\n")] = '\0';

    int encontrado = 0;
    for (int i = 0; i < qtd_investidores; i++)
    {
        if (strcmp(investidores[i].nome, nome_investidor) == 0)
        {
            encontrado = 1;
            break;
        }
    ...
    {
        // Verifica se a linha contem o nome do investidor
        if (strstr(linha, nome_investidor))
        {
            printf("%s", linha);
        }
    }
